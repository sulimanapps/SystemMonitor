...
class FeatureManager: ObservableObject {
    ...
    func cleanRAM() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let beforePipe = Pipe()
            beforeTask.standardOutput = beforePipe
            beforeTask.standardError = FileHandle.nullDevice

            var freePagesBefore: UInt64 = 0
            ...
            // Run memory_pressure to free inactive memory
            let task = Process()
            ...
            do {
                ...
                afterTask.launchPath = "/usr/bin/vm_stat"
                let afterPipe = Pipe()
                afterTask.standardOutput = afterPipe
                ...
                DispatchQueue.main.async {
                    self?.lastRAMCleanResult = "Optimization complete"
                    self?.isCleaningRAM = false
                }
            }
        }
    ...
    private func parseFreePages(_ vmstatOutput: String) -> UInt64 {
